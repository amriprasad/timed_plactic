...
class TimedTableau(TimedWord):
    ...
    def inflate(self, la, m):
        l = len(la)
        mu0 = self.shape()
        mu = mu0 + [0]*(l-len(mu0))
        rows = [TimedRow([])]*(l-len(mu0)) + self.rows() 
        output = TimedWord([])
        for i, r in enumerate(rows):
            output = output.concatenate(r).concatenate(TimedWord([[m, la[l-i-1]-mu[l-i-1]]]))
        return TimedTableau(output)
...
def knuth_rsk(A):
    ...
    m, n = A.shape
    for i in range(m):
        P = P.insert_row(TimedRow([[j+1, A[i,j]] for j in range(n)]))
        Q = Q.inflate(P.shape(), i+1)
    return P, Q

def knuth_dual_rsk(A):
    P = DualTimedTableau([])
    Q = TimedTableau([])
    m, n = A.shape
    for j in range(n):
...
class DualTimedTableau():
    ...
    def inflate(self, la, i):
        ...
        for k, row in enumerate(self._rows):
            lr = row.length()
            output.append(row.concatenate(TimedRow([[i, la[k]-lr]])))
        output += [TimedRow([[i, la[k]]]) for k in range(m,l)]
        return DualTimedTableau(output)

def conjugate(real_partn):
    n = ceil(real_partn[0])
    sumtillnow=0
    output = []
    for i in range(n):
        mini = sum([min(part, i+1) for part in real_partn])
