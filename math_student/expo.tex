...
\begin{proof}
  ...
  Write $v_1'=xy$, where $x$ consists of segments that come from $u_{1[0,l(u_2))}$ and $y$ consists of segements that come from $u_{1[l(u_2),l(u_1))}$.
...
\begin{corollary}
  ...
  \begin{multline*}
    \rowins: \tr_n(r)\times \tr_n(s) \tilde\to \\\{(v',u')\in R_n(s')\times R_n(r+s-s')\mid 0\leq s'\leq \min(r,s),\; u'\rhd w'\}. 
  \end{multline*}
\end{corollary}
\begin{proof}
  ...
  Then using Lemma~\ref{lemma:reverse-row-insertion}, $u$ and $v$ can be recovered as $u&=u_1$, and $v=v_1u'_{[r,r+s-s')}$.
\end{proof}
