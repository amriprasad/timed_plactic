...
\begin{lemma}
  \label{lemma:concatenation-domination}
  If $u_1$, $u_2$, $v_1$, and $v_2$ are rows such that $u_1u_2$ and $v_1v_2$ are also rows.
  Then if $u_1\lhd v_1$ and $u_2\lhd v_2$, then $u_1u_2\lhd v_1v_2$.
\end{lemma}
\begin{proof}
  We have $l(u_1u_2)=l(u_1)+l(u_2)\geq l(v_1)+l(v_2)=l(v_1v_2)$.
  Also, for $0\leq t <l(v_1)$, $v_1v_2(t)=v_1(t)>u_1(t)=u_1u_2(t)$.
  On the other hand, if $l(v_1)\leq t <l(v_1v_2)$, then 
  \begin{align*}
    v_1v_2(t) &=    v_2(t-l(v_1))\\ 
              &>    u_2(t-l(v_1))\\
              &=    u_1u_2(t-l(v_1)+l(u_1))\\
              &\geq u_1u_2(t),
  \end{align*}
  proving the lemma.
\end{proof}
\begin{lemma}
  \label{lemma:subword-domination}
  Let $u$ and $v$ be rows, and $u'$ be a subword of $u$.
  If $u'\lhd v$, then $u\lhd v$. 
\end{lemma}
\begin{proof}
  We have $l(v)<l(u')\leq l(u)$.
  Also, $v(t)>u'(t)\geq u(t)$.
\end{proof}
\begin{lemma}
  \label{lemma:dom1.5}
  Let $u$ and $v$ be rows, and $(v',u')=\rowins(u,v)$.
  Then $u'\lhd v'$.
\end{lemma}
\begin{proof}
  As the terms of $v$ are inserted into $u$, successive segments (possible even empty ones) are displaced from $u$.
  These displaced segments are concatenated to form $v'$.
  Each of these segments has initial value strictly larger than the value of the term that has displaced it.
  Therefore, each such segment dominates the term that has displaced it.
  By Lemma~\ref{lemma:concatenation-domination}, $v'$ dominates a subowrd of $u'$, and so by Lemma~\ref{lemma:subword-domination}, it also dominates $u'$.
\end{proof}
\begin{lemma}
  \label{lemma:dom1}
  Let $u$ and $v$ be rows and $(v',u')=\rowins(u,v)$.
  Then $l(v')\leq l(v)$, and  $v'(t)\geq v(t)$ for $0\leq t <l(v')$.
\end{lemma}
\begin{proof}
  The proof is similar to that of Lemma~\ref{lemma:dom1.5}.
\end{proof}
\begin{lemma}
  Let $u_1$, $v_1$, $u_2$, and $v_2$ be timed rows such that
  $u_1\lhd u_2$, $l(v_1)\geq l(v_2)$, and $v_1(t)\leq v_2(t)$ for all $0\leq t<l(v_2)$.
  Suppose $(v_i',u_i')=\rowins(u_i,v_i)$ for $i=1,2$.
  Then $u_1'\lhd u_2'$.
\end{lemma}
\begin{proof}
  Assume that $v_1=c_1^t$ and $v_2=c_2^t$ for some $t>0$, and $c_1\leq c_2$.
  Suppose that the segments corresponding to $[t_1,t_1+t_1')$ and $[t_2,t_2+t_2')$ are bumped out by these insertions, and replaced by segments with values $c_1$ and $c_2$ respectively.
  Since $[t_2,t_2+t_2')$ is the only segment where the value of $u_2'$ is less than the value of $u_2$ (and $u_1'(t)\leq u_1(t)$ for all $t$), in order to show that $u_1\lhd u_2$, this is the only region of interest.
