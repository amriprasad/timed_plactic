...
%      3^{0.8}1^{0.5}4^{1.1}1^{0.9}2^{1.6}3^{0.7}1^{0.7} & 3^{0.7}2^{0.7}3^{0.1}4^{1.1}1^{2.1}2^{0.9}3^{0.7}\\
  ...
  \end{displaymath}
\end{example}
\subsection{Timed Greene Invariants}
\label{sec:timed-greene-invar}
\begin{definition}[Timed Greene Invariants]
  \label{definition:timed-Greene-invars}
  Given a word $w\in A_n^\dagger$, its $k$th Greene invariant $a_k(w)$ is defined to be the maximum possible sum of lengths of a set of $k$ pairwise disjoint subwords of $w$ whose elements are all rows:
  \begin{multline*}
...
\begin{proof}
  ...
  \end{displaymath}
  For $j=1,\dotsc, l$, set $r_j=\lambda_1+\dotsb + \lambda_{j-1}$.
  Then $[0,r)$ is partitioned into segments $T_j=r_j+[0,\lambda_j)$, for $j=1,\dotsb,l$.
  Note that $u_i(t)=w(r_j+t)$, so that the condition $u_i(t)>u_j(t)$ for $j<i$ can be rewritten as $w(r_j+t)>w(r_i+t)$ if $j<i$.
  Therefore, if $v$ is the row subword of $w$ with respect to $S$, then $S$ cannot simultaneously contain $r_j+t$ and $r_i+t$ for $j<i$.
  It follows that the sets $(S-r_j)\cap [0,\lambda_j)$ are pairwise disjoint for $j=1,\dotsc,l$.
  Suppose that $v_1,\dotsc,v_k$ are pairwise disjoint subwords of $w$ supported on the pairwise disjoint subsets $S_1,\dotsc,S_k$ of $[0,r)$.
  Consider the subsets $T_{ij} = (S_i-r_j)\cap [0,\lambda_j)$.
  Define 


\end{proof}
\subsection{Timed Knuth Equivalence}
\begin{definition}
