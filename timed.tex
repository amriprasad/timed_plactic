...
\begin{lemma}
  ...
  Then $v$ and $w$ differ by a Knuth move (\ref{eq:tk1}) if and only if $v^\sharp$ and $w^\sharp$ differ by a Knuth move (\ref{eq:tk2}).
\end{lemma}
\begin{proof}
...
\begin{lemma}
  Let $w, u, v$ be rows in $A_n$, and assume that 
\end{lemma}
\begin{lemma}
  The row insertion algorithm gives rise to a bijection:
  \begin{multline*}
...
\begin{lemma}[Pieri's Rule]
  \label{lemma:pieri}
  The insertion map gives rise to a bijection:
  \begin{displaymath}
