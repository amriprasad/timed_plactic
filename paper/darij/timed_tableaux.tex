...
\begin{definition}
  ...
  Given a timed word $w:[0,r)\to A_n$, and $S\subset [0,r)$ a finite disjoint union of intervals of the form $[a, b)\subset [0,r)$, the timed subword of $w$ with respect to $S$ is the timed word $w_S$ defined by:
  \begin{displaymath}
...
\begin{definition}[Timed row insertion]
  ...
  Given a timed row $u$, a letter $c\in A_n$, and a real number $t\geq 0$, the insertion $\rowins(u, c^{t_c})$ of $c^{t_c}$ into $u$ is defined as follows:
  ...
  \begin{displaymath}
    ...
    \end{cases}
  \end{displaymath}
  When $t_c=1$ and $u$ lies in the image of $A^*$ in $A^\dagger$, this coincides with the first step of the algorithm INSERT from Knuth \cite{knuth} where $c$ is inserted into a row:
