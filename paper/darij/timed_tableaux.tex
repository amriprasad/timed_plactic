...
\begin{remark}
  ...
  Therefore, in order that row insertion be well-defined for timed words, it is necessary to check that the result of row insertion of $c^{t_1+t_2}$ is the same as the result of row insertion of $c^{t_1}$ followed by row inertion of $c^{t_2}$.
\end{remark}
\begin{example}
...
\begin{proof}
  ...
  \begin{proof}[Proof of the lemma]
    ...
    If $x$ never exceeeds $c$, then $x'=xc^t$, and $y'=y$, and so $x'\lhd y'$.
  ...
  Supppose $w$ has row decomposition $u_lu_{l-1}\dotsb u_1$.
...
\begin{proof}
  ...
  If $x_i=\emptyset$ for all $i=1,\dotsc,r$, or $z_i=\emptyset$ for all $i=1,\dotsc,r$, then all the $v_i$'s remain row subwords of $wzxyu$, and so $a_k(wzxyu)\geq a_k(wxzyu)$.
  Assume that $z_1\neq \emptyset$. Then $yu_1$ is a row.
  ...
  There remains the case where at least one $x_i\neq \emptyset$ for $i=1,\dotsc,r$, at least one $y_i\neq \emptyset$ for $i=r+1,\dotsc,k$, and at least one $z_i\neq \emptyset$ for $i=1,\dotsc,r$.
  Let $a$ be the index for which $x_i$ has the smallest first letter among all $i=1,\dotsc,k$.
  Let $b_1$ (resp., $b_2$) be the index for which $y_i$ has the largest last letter (resp., smallest first letter) among all $i=r+1,\dotsc,k$.
  Let $c$ be the index for which $z_i$ has the largest last letter among all $i=1,\dots,r$.

  Form the timed row $x_0$ (respectively, $y_0$ and $z_0$) by concatenating all the segments of all the $x_i$ (respectively, $y_i$ and $z_i$) in the order in which they occur in $x$ (respectively, $y$ and $z$).
  Take:
  \begin{align*}
    v'_a &= w_ax_0y_0u_b,\\
    v'_b &= w_bu_a \text{(only if }\\
    v'_c &= w_cz_0u_c.
  \end{align*}
