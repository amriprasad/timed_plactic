...
It is now well-accepted that these bijections are restrictions to lattice points of certain volume-preserving piecewise linear bijections between convex polyhedra \cite{berkir,kir-trop,pak}.
...
Our goal is to lay the monoidal foundations for piecewise linear correspondences interpolating bijections involving tableaux.
...
An expository article on the classical version of Greene's theorem, and its extension to the timed case (based on the lecture) has been submitted to \emph{The Mathematics Student} \cite{math-student}.
